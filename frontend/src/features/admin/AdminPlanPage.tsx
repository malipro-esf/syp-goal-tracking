...
const transitions: Record<string, string[]> = { draft: ['active', 'archived'], active: ['paused', 'completed', 'archived'], paused: ['active', 'completed', 'archived'], completed: ['active', 'archived'], archived: [] }
...
export function AdminPlanPage() {
  ...
  async function changeStatus(status: string) {
    if (!window.confirm(`Change “${plan?.title}” from ${plan?.status} to ${status}?`)) return
    setSaving(true)
    try {
      const updated = await apiRequest<PlanDetail>(`/api/v1/admin/plans/${planId}/status`, { method: 'PATCH', headers: { Authorization: `Bearer ${accessToken}` }, body: JSON.stringify({ status }) })
      setPlan(updated); setMessage(`Plan status changed to ${status}.`); setError('')
    } catch (caught) { setError(caught instanceof ApiError ? caught.message : 'Plan status could not be changed.'); setMessage('') }
  ...
  return <AdminLayout active="plans" title={plan.title} description="Read-only administrative inspection.">
    ...
    <section className="card admin-plan-controls"><div><p className="eyebrow">Administrative control</p><h2>Plan status</h2><p>Every status change is recorded in the audit log.</p></div><div className="lifecycle-actions">{transitions[plan.status].map((status) => <button type="button" className={status === 'archived' ? 'secondary-button' : ''} disabled={saving} onClick={() => changeStatus(status)} key={status}>{status === 'active' ? 'Activate' : status === 'paused' ? 'Pause' : status === 'completed' ? 'Complete' : 'Archive'}</button>)}{transitions[plan.status].length === 0 && <span className="status-badge status-archived">No further transitions</span>}</div></section>
