...
export function AdminPage() {
  ...
  return <AdminLayout active="overview" title="Admin dashboard" description="Monitor accounts and application activity.">
    ...
    {alerts && <section className="card admin-alerts"><div className="section-heading"><div><p className="eyebrow">Needs attention</p><h2>Operational alerts</h2></div></div><div className="admin-alert-grid"><Link to="/admin/plans?status=active"><strong>{alerts.expired_active_plans}</strong><span>Expired active plans</span></Link><Link to="/admin#users"><strong>{alerts.disabled_users_with_active_plans}</strong><span>Disabled users with active plans</span></Link><Link to="/admin/assignments?stale=true"><strong>{alerts.stale_pending_invitations}</strong><span>Invitations pending {alerts.stale_after_days}+ days</span></Link></div></section>}
