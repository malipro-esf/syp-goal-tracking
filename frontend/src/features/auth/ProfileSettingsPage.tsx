...
export function ProfileSettingsPage() {
  ...
  return <main className="page-shell profile-page"><header className="page-header"><div><p className="eyebrow">{t('profile.eyebrow')}</p><h1>{t('profile.title')}</h1><p>{t('profile.description')}</p></div></header>
    <div className="profile-grid"><form className="card profile-form" onSubmit={submit}>
      ...
      <label>{t('profile.language')}<select name="language" defaultValue={user.preferred_language}>{supportedLanguages.map(code => <option key={code} value={code}>{languageNames[code]}</option>)}</select></label>
      <label>{t('profile.timezone')}<input name="timezone" defaultValue={user.timezone} required /><small>{t('profile.timezoneHint')}</small></label>
    ...
    </form><aside className="card account-summary"><h2>{t('profile.account')}</h2><dl><div><dt>{t('profile.email')}</dt><dd>{user.email}</dd></div><div><dt>{t('profile.roles')}</dt><dd>{user.roles.join(', ')}</dd></div></dl><p>{t('profile.accountHint')}</p></aside></div>
  </main>
}
