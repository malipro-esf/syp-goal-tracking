...
export type User = { id: string; email: string; display_name: string; bio: string | null; timezone: string; preferred_language: 'en' | 'fa' | 'tr' | 'ar' | 'de' | 'el' | 'ja' | 'zh-CN' | 'es' | 'sv' | 'fr' | 'pt-BR' | 'hi' | 'ko' | 'fi'; gender: 'man' | 'woman' | null; gender_theme_enabled: boolean; roles: string[] }
